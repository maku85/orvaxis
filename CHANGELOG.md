...
- **`otelPlugin` now creates child spans for pipeline and handler stages** — each request produces three spans instead of one: the root `SERVER` span (unchanged) plus `orvaxis.pipeline` and `orvaxis.handler` child spans. `orvaxis.pipeline` opens at `beforePipeline` and closes at `beforeHandler`, covering global pipeline middleware and group/route middleware chains. `orvaxis.handler` opens at `beforeHandler` and closes at `afterHandler`, isolating handler time from middleware overhead. Both child spans are parented to the root span via the stored OTel context (`trace.setSpan`), so they appear correctly nested in Jaeger, Zipkin, and any W3C-compliant backend. On any error path (policy denial, middleware throw, handler throw, or 404/405) the plugin closes any open child spans before ending and marking the root span as `ERROR`. Requests that fail before routing (404/405) still produce only the root span. No configuration changes required — existing registrations (`app.register(otelPlugin({ tracer }))`) get child spans automatically.
...
- **`withTimeout` is now used by both adapters — no more inline duplication** — both `createExpressServer` and `createFastifyServer` previously contained an identical copy of the `Promise.race + AbortController + setTimeout` pattern instead of calling the `withTimeout` utility that already existed in `http/timeout.ts`. The copies differed slightly (streaming timer cancellation), which was a maintenance hazard. `withTimeout` gains an optional fourth parameter `onCancel?: (cancel: () => void) => void`; the adapters pass a callback that stores the cancel function and call it from `onStreamStart` to disable the deadline when SSE/chunked streaming begins. `HttpError` is no longer imported by either adapter directly.

- **`PluginManager` now rejects duplicate plugin names** — `register()` throws a `TypeError` if a plugin with the same `name` is already registered, e.g. `Plugin "logger" is already registered`. Previously the second registration would silently double every hook listener, causing duplicate log lines, doubled OTel spans, and other hard-to-diagnose production bugs. Fails immediately at startup — not on first request.
